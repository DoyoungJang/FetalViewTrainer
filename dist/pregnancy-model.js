...
export function createPregnancyModel(bodyMeshes,uterusAsset){
 ...
 const placenta=mesh(new T.SphereGeometry(1,48,32),material(0xb9576e),'pregnancy-placenta');placenta.position.set(0,.65,-1.39);placenta.scale.set(.54,.55,.075);tissue.push(placenta);
 const cord=mesh(new T.TubeGeometry(new T.CatmullRomCurve3([[0,.65,-1.315],[.42,.3,-.6],[.38,-.05,-.15],[0,.4,.1]].map(p=>new T.Vector3(...p))),48,.045,12,false),material(0xb4d5cf),'pregnancy-cord');
 ...
 const preset=type=>({center:new T.Vector3(0,type==='cervix'?-2.1:-.55,type==='cervix'?0:-.65),normal:new T.Vector3(1,0,0),landmarks:type==='cervix'?[new T.Vector3(0,-1.7,0),new T.Vector3(0,-2.5,0)]:[new T.Vector3(0,.10,-1.39),new T.Vector3(0,-1.7,0)],anchorNames:type==='cervix'?['합성 장면의 내자궁구','합성 장면의 외자궁구']:['태반 하연 예시','내자궁구 예시'],extent:type==='cervix'?2.8:4.8});
