...
export function renderMeasurements(v){const p=measurementsFor(v);if(!p)return '';return `<section class="measurement-guide" aria-label="주수별 측정 항목과 캘리퍼 배치"><h3>정확한 캘리퍼(측정자) 배치 기준 <small>${p.trimester}분기 · 주수별 측정 항목</small></h3><p class="measurement-intro">기본 계측 · 추가/정밀 계측 · 형태 평가를 구분했습니다. 아래 주수 범위를 먼저 확인하세요. 방향은 화면이 아닌 해부학적 축을 기준으로 합니다.</p>${p.rows.map(r=>`<article class="measurement-item"><h4>${escape(r.name)}</h4><span class="measurement-role">${escape(r.role)}</span><dl><dt>적용 주수</dt><dd>${escape(r.weeks)}</dd><dt>캘리퍼·측정 방법</dt><dd>${escape(r.placement)}</dd><dt>해석·주의</dt><dd>${escape(r.interpretation)}</dd></dl></article>`).join('')}<p class="measurement-note">길이는 mm, 심박수는 bpm, 비율은 사용한 정의에 맞춰 기록하고 주수·단면·측정법을 함께 남깁니다. 정상 범위 안의 한 측정값만으로 전체 구조가 정상임을 확정하지 않습니다.</p><div class="measurement-sources">${p.refs.map(([name,url])=>`<a href="${url}" target="_blank" rel="noopener">${name} ↗</a>`).join('')}</div></section>`;}
