...
export function heartModelPreset(asset,type){
 ...
 if(['threev','threevpa'].includes(type)){
  const names=['pulmonary_trunk','ascending_aorta','superior_vena_cava'];
  ...
  const level=lower+(upper-lower)*(type==='threevpa'?.60:.70);
  ...
  return {center:points.reduce((a,p)=>a.add(p),new T.Vector3()).multiplyScalar(1/3),normal:new T.Vector3(0,1,0),landmarks:points,names,method:'Shared vascular cross-section level'};
 }
 const point=name=>{
