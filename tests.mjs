...
let registered;const storage=new Map();const sandbox={document:{querySelector:el,modelContext:{registerTool:t=>registered=t}},localStorage:{getItem:k=>storage.get(k),setItem:(k,v)=>storage.set(k,v)},phases,sources,drawExtra,renderUltrasound,console,Set,Promise};
let code=await readFile('dist/app.js','utf8');code=code.replace("import {renderUltrasound} from './ultrasound.js?v=24';",'').replace("import {phases,sources} from './data.js?v=24';",'').replace("import {drawExtra} from './extra-diagrams.js?v=24';",'').replace(/setup3D\(\)\.catch\(\(\)=>\{.*?\}\);/s,'');vm.runInNewContext(code,sandbox);
